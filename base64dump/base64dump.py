    ...
    if options.yara != None:
        ...
        for encoding in dEncodings:
            for encodeddata, decodeddata in dEncodings[encoding][1](data):
                ...
                line = '%3s: %7d %-16s %-16s %s' % (
                encoding, len(encodeddata), encodeddata[0:16], AsciiDump(decodeddata[0:16]),
                hashlib.md5(decodeddata).hexdigest())
        ...
        for encoding in dEncodings:
            for encodeddata, decodeddata in dEncodings[encoding][1](data):
                ...
                if options.unique and decodeddata in dDecodedData:
                    continue
                dDecodedData[decodeddata] = True
                report.append([len(encodeddata), '%3s: %7d %-16s %-16s %s' % (
                encoding, len(encodeddata), encodeddata[0:16], AsciiDump(decodeddata[0:16]),
                hashlib.md5(decodeddata).hexdigest())])
